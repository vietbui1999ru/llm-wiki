---
title: "Commandr"
description: "~/repos/Commandr — the L3 bus in the 5-layer agent toolchain. A filesystem contract plus the shell tools that operate it. Not a monolith app — identity is wa…"
---

# Commandr

`~/repos/Commandr` — the **L3 bus** in the [5-layer agent toolchain](/syntheses/desktop-control-plane). A filesystem contract plus the shell tools that operate it. Not a monolith app — identity is waist and services.
...
- [Agent-Native](/entities/agent-native) is action/state inspiration for the UI, not a replacement for `.agents/`.
- Builderio Skills are workflow packages consumed by runners, not lifecycle state.
- [omp (oh-my-pi)](/entities/omp) is an L2 worker, not the owner of claims, approvals, events, or final task status.
...
- [Desktop AI Agent Control Plane — Architecture Synthesis](/syntheses/desktop-control-plane) — big-picture synthesis; Tauri path; where omp fits
- [DiffViewer](/entities/diffviewer) — L5 UI that reads the bus
- [Pi Agent (pi-mono)](/entities/pi-agent) — L2 execution substrate; council subprocess
- [omp (oh-my-pi)](/entities/omp) — potential L2 alternative; batteries-included Pi fork
- [Agent Harness](/concepts/agent-harness) — harness engineering context
- [Control Plane Expansion Plan — Gap Analysis and Phase 0.5 Roadmap](/syntheses/control-plane-expansion-plan) — earlier gap analysis (pre-Unification Blueprint; partially superseded)
