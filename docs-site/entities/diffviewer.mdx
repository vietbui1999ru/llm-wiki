---
title: "DiffViewer"
description: "~/repos/DiffViewer — the L5 UI in the 5-layer agent toolchain. Real-time diff review tool for agent-generated file changes, with steer injection and phone ap…"
---

# DiffViewer

`~/repos/DiffViewer` — the **L5 UI** in the [5-layer agent toolchain](/syntheses/desktop-control-plane). Real-time diff review tool for agent-generated file changes, with steer injection and phone approval. Currently a Node.js/Hono browser server; Phase 5 target = Tauri desktop app.
...
DiffViewer/Tauri owns the human-facing projection and local SQLite audit view. [Commandr](/entities/commandr) remains the lifecycle source of truth.
...
- [Desktop AI Agent Control Plane — Architecture Synthesis](/syntheses/desktop-control-plane) — big-picture synthesis; Tauri path
- [Commandr](/entities/commandr) — L3 bus; approval token format; events.jsonl
- [omp (oh-my-pi)](/entities/omp) — potential L2 worker with built-in browser + eval kernel; could replace Pi extension
- [Agent-Native](/entities/agent-native) — action/state philosophy for the cockpit
- Builderio Skills — visual-plan/visual-recap artifact pattern
- [Agent Diff Viewer — Real-Time Code Review Tool](/syntheses/agent-diff-viewer) — earlier synthesis (pre-bus-watching, pre-mobile, pre-Pi-extension; partially superseded)
- [Agent Harness](/concepts/agent-harness) — hook system, PostToolUse event shape
