...
See also [ketch](/entities/ketch) — stateless CLI covering scrape/crawl plus web/code/docs search in one binary; narrower per-surface than Firecrawl (no schema extraction or autonomous agent mode) but config-driven backend swap and no daemon/cloud dependency.
