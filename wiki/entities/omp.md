...
### Snapcompact
Bitmap-frame context compression — distinct from summarization-based compaction. Renders discarded conversation history into dense PNG frames of pixel-font glyphs that vision models read back directly. Local and deterministic — no LLM call, no API key.

**Key detail for tool outputs:** snapcompact already truncates tool results during archival compaction:
...
This is **conversation-level** archival compression (old turns → PNG frames). It complements but does not replace per-turn tool output truncation. See [[research/omp-snapcompact-rpc]] for full research.
...
|---|---|---|
| Interactive TUI | `omp` | Daily coding sessions |
| One-shot | `omp -p "..."` | CI, quick tasks |
| Node/TS SDK | `@oh-my-pi/pi-coding-agent` | Embedding in Node apps |
| NDJSON RPC | `omp --mode rpc` | **Headless automation, bus integration, polyglot host tools** |
| ACP (Zed) | `omp acp` | Editor integration |
...
| Scope | Full agent lifecycle + UI | Tools only |
