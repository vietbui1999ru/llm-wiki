...
| `commandr-omp-runner` | Scaffold/smoke only | L2 runner wrapper target; RPC host tools needed for real bus-native progress and approvals |
...
- **Bootstrap** (`setup.sh`): installs omp, copies hooks/tools to agent dirs
...
Research on `omp --mode rpc` reveals a better path than custom TypeScript plugins for bus integration:

| Integration Approach | Effort | Language Lock-in | Bidirectional |
|---|---|---|---|
| Custom TS plugins (Level 3) | High | TypeScript only | Limited |
| **RPC host tools (Level 2+)** | **Medium** | **None — any JSON-lines process** | **Full** |
| Subprocess + parse stdout (Level 1) | Low | None | One-way |

**RPC host tools** let Commandr register `commandr_progress`, `commandr_request_approval`, etc. as host-side tools. The agent calls them via standard tool calling; the host (our runner) handles the bus side effects. No TypeScript required — the host can be Python, Rust, Go, or bash.

**Recommended evolution:**
- Phase 1 (current): `--mode json`, one-shot scaffold/smoke path
- Phase 2: `--mode rpc`, register host tools for bus-native progress/approvals/artifacts
- Phase 3: `--mode rpc-ui`, handle `extension_ui_request` for approval flows

See [[research/omp-snapcompact-rpc]] for full protocol analysis.
